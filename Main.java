			...
			if (i < conj.length) {
				...
				case 1: // Cadastrar uma venda
					try {
						...
						v = (Vendedor) persist.lerVendedor(nvd); // Tenta Ler o
						// vendedor

						// Tentar pegar o nome, caso seja nulo irá entrar no
						// catch
						try {
							System.out.println(v.getNome()); // Se existir ele
							// irá pular
							// diretamente
							// para o
							// Cliente
						...
						try {
							...
							do {
								...
								if (conj[i].adicProduto(p) == false) {
									JOptionPane.showMessageDialog(null,
											"Erro: Atingiu o limite max de produtos na venda.");
								}
								...
								op1 = JOptionPane.showInputDialog("Vender outro produto? (S/N)");
								while ((!op1.equalsIgnoreCase("S") && !op1.equalsIgnoreCase("N"))) {
									op1 = JOptionPane.showInputDialog("Valor inválido.\nVender outro produto? (S/N)");
								}

							} while (op1.equalsIgnoreCase("S")); // fim
							// do
							// Do
							// While
							...
							v.setComissao(aux);
							persist.salvarVendedor(v, v.getNome()); // atualizar
							// vendedor
				...
				case 13: // Colocar produto em promocao
					try {
						...
						p = (Produto) persist.lerProduto(npd);
						JOptionPane.showMessageDialog(null, "Valor atual:\n" + p.getVal());
						p.setValPromocao(
